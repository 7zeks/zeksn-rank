    ...
    createSimpleRow(item, index) {
        ...
        if (this.isAdmin()) {
            ...
            deleteBtn.onclick = () => {
                ...
                const deletedName = item.name; // Zapisujemy nazwę do powiadomienia
                
                list.splice(index, 1); // Usuwa element od razu
                this.save(); // Zapisuje zmiany w bazie i odświeża widok
                
                // Wyświetla Twoje ładne powiadomienie zamiast okienka przeglądarki
                this.showNotification(`Usunięto: ${deletedName}`); 
            };
